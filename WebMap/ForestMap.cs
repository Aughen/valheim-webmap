...
namespace WebMap
{
    ...
    internal static class ForestMap
    {
        ...
        private static string Percentiles(float[] blur)
        {
            ...
            if (v.Count == 0) return "{}";
            v.Sort();
            var inv = System.Globalization.CultureInfo.InvariantCulture;   // "0,50" is not JSON
            System.Func<float, float> q = p => v[Mathf.Clamp((int)(p * v.Count), 0, v.Count - 1)];
            return "{\"p50\":" + q(0.50f).ToString("0.00", inv)
                 + ",\"p90\":" + q(0.90f).ToString("0.00", inv)
                 + ",\"p99\":" + q(0.99f).ToString("0.00", inv)
        ...
        public static byte[] GetPng()
        {
            ...
            lock (encodeLock)
            {
                ...
                png = Util.Png.Encode(rgba, size, size, Util.Png.Format.RGBA, fast: true);
                pngStale = false;
                return png;
            }
        }
