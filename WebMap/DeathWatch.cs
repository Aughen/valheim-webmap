...
namespace WebMap
{
    ...
    internal class DeathWatch
    {
        ...
        private static void Announce(RoutedRPCData rpcData)
        {
            ...
            try
            {
                ZDO zdo = characterID.IsNone() ? null : ZDOMan.instance.GetZDO(characterID);
                if (zdo != null) { var p = zdo.GetPosition(); x = p.x; z = p.z; }
            }
            catch { }
            Live.Events.Add("death", name, "died", x, z);
            Live.Stats.OnDeath(name);
        }
